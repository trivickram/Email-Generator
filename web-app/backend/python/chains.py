...
class Chain:
    def __init__(self):
        ...
        try:
            # Use the correct parameter name for newer versions
            self.llm = ChatGroq(
                temperature=0, 
                api_key=groq_api_key,  # Use 'api_key' instead of 'groq_api_key'
                model="llama-3.1-70b-versatile"  # Use 'model' instead of 'model_name'
            )
            ...
            fallback_configs = [
                ...
                {"temperature": 0, "api_key": groq_api_key, "model": "llama3-70b-8192"},
                {"temperature": 0, "groq_api_key": groq_api_key, "model_name": "mixtral-8x7b-32768"},
                {"temperature": 0, "api_key": groq_api_key, "model": "llama3-8b-8192"}
            ]
            
            for i, config in enumerate(fallback_configs):
                try:
                    logging.info(f"Trying fallback configuration {i+1}: {config}")
                    self.llm = ChatGroq(**config)
                    logging.info(f"✅ Fallback configuration {i+1} successful!")
                    break
                except Exception as fallback_error:
                    logging.error(f"Fallback {i+1} failed: {fallback_error}")
                    continue
            else:
                ...
                if "401" in str(e) or "authentication" in str(e).lower():
                    ...
                    raise EnvironmentError(f"Failed to initialize any LLM configuration. Last error: {e}")
