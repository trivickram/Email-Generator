...
const app = express();
const PORT = process.env.PORT || 4000; // Changed from 5000 to 4000 to match frontend
...
const limiter = rateLimit({
  ...
  trustProxy: true, // Trust proxy headers (required for Render)
...
app.listen(PORT, () => {
  ...
  console.log(`📧 Cold Email Generator API ready`);
  console.log(`🌐 Environment: ${process.env.NODE_ENV || 'development'}`);
  console.log(`🔗 Health check: http://localhost:${PORT}/api/health`);
  console.log(`🧠 API docs: http://localhost:${PORT}/`);
  console.log(`🐍 Python path: ${process.env.PYTHON_PATH || 'system default'}`);
});
